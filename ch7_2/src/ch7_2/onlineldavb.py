...
class OnlineLDA:
    ...
    def add_new_features(self, new_length):
        ...
        """
        self._W = new_length
        # new_lambda = np.ones((self._lambda.shape[0], new_length))/self._lambda.shape[1]     #  uniformly set new code words to 1/len(codebook)
        # new_lambda = 1*np.random.gamma(100., 1./100., (self._K, self._W))                   #  uniformly set new code words to random gamma
        new_lambda = np.zeros([self._lambda.shape[0], new_length])   + 0.0000001              #  uniformly set new code words to (very close to) 0
    ...
    def add_new_topics(self, plus_n_topics=1):
        """Add Topics incrementally, as this might help if you don't observe all activities in the first day etc.
        """
        self._K += plus_n_topics
        # new_lambda = np.zeros([self._K, self._lambda.shape[1]])   + 0.0000001              #  uniformly set new code words to (very close to) 0
        # new_lambda = 1*np.random.gamma(100., 1./100., (self._K, self._W))                   #  uniformly set new code words to random gamma
        new_lambda = np.ones((self._K, self._W))/self._W     #  uniformly set new code words to 1/len(codebook)

        if plus_n_topics>0:
            new_lambda[:-plus_n_topics,:] = self._lambda
            self._lambda = new_lambda
        self._Elogbeta = dirichlet_expectation(self._lambda)
        self._expElogbeta = np.exp(self._Elogbeta)
    ...
    def update_lambda(self, wordids, wordcts, dbg=False):
        """
        ...
        bound = self.approx_bound(wordids, wordcts, gamma, dbg)
    ...
    def approx_bound(self, wordids, wordcts, gamma, dbg):
        """
