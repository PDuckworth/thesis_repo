...
def print_results(true_labels, pred_labels, num_clusters):
    ...
    norm_gt = []
    for i in mat:
        norm_gt.append([float(j)/float( sum(i, 0) ) for j in i ])
    ...
    ax.imshow(norm_gt, interpolation='nearest')
    plt.show()
    ...
    plt.setp((ax), xticks=range(len(mat[0])), yticks=xrange(len(set_of_true_labs)))
    plt.yticks(xrange(len(set_of_true_labs)), set_of_true_labs, color='red')
    ax.imshow(reordered_mat, interpolation='nearest')
    plt.show()
    ...
    plt.setp((ax), xticks=range(len(mat[0])), yticks=xrange(len(set_of_true_labs)))
    plt.yticks(xrange(len(set_of_true_labs)), set_of_true_labs, color='red')
    ax.imshow(norm_topics[:, ordering], interpolation='nearest')
    plt.show()
